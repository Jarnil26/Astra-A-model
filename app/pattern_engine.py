...
class PatternEngine:
    ...
    def detect_patterns(self, input_symptoms):
        """Detect matched patterns based on symptom threshold (>70% or >=2)."""
        s_input = set(s.lower().strip() for s in input_symptoms)
        active_patterns = []
        
        for p in self.PATTERNS:
            p_symptoms = set(s.lower().strip() for s in p["symptoms"])
            matched = s_input.intersection(p_symptoms)
            match_count = len(matched)
            
            # Threshold: >= 2 or 70% of pattern symptoms
            if match_count >= 2 or (match_count / len(p_symptoms)) >= 0.7:
